...
@testset "Symmetries of fermionic cluster expansion" begin
    ...
    for (key, value) in O_symm
        @test norm(O_symm[key]) - norm(O_asymm[key]) < 1.0e-14
    end
end
...
@testset "Loop solver" begin
    ...
    @test error_asymm < 1.0e-3
    @test error_symm < 2.0e-2
    @test norm(ClusterExpansions.rotl90_fermionic(As_symm[1]) - As_symm[4]) < 1.0e-14
end
