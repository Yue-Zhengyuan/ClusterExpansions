...
function apply_isometry(A::Tuple{AbstractTensorMap{E, S, 2, 4}, AbstractTensorMap{E, S, 2, 4}}, Ws::Vector{<:AbstractTensorMap{E, S}}, inds::Vector{Int}) where {E, S}
    A_trunc = ncon([A[1], A[2], [Ws[i] for i in setdiff(1:4, inds)]...], [[1, -2, [i ∈ inds ? -i - 2 : 2 * i for i in 1:4]...], [-1, 1, [i ∈ inds ? -i - 6 : 2 * i + 1 for i in 1:4]...], [dir > 2 ? [-dir - 2, 2 * dir, 2 * dir + 1] : [2 * dir, 2 * dir + 1, -dir - 2] for dir in setdiff(1:4, inds)]...], fill(false, 6 - length(inds)))
