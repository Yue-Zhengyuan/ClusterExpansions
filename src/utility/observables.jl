...
function localoperator_model(pspace, op::Tuple{TensorMap{T, S, 1, 1}, TensorMap{T, S, 2, 2}}; Nr = 1, Nc = 1) where {T, S}
    ...
    return PEPSKit.LocalOperator(
        pspaces, (neighbor => op[2] for neighbor in PEPSKit.nearest_neighbours(lattice))...,
...
function calculate_observables_purification(O::AbstractTensorMap{E, S, 2, 4}, χ::Int, observables_base) where {E, S}
    ...
    return [expectation_value(peps, obs, ctmrg_env) for obs in observables]
end
...
function calculate_observables(ψ::InfinitePEPS, χ::Int, observables)
    ...
    return [env_type == :VUMPS ? expectation_value(ψ, obs.obs, vumps_env) : (env_type == :CTMRG ? expectation_value(ψ, obs.obs, ctmrg_env) : obs.obs(ψ)) for (obs, env_type) in zip(observables, env_algs)]
end
